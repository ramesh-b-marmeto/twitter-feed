...
const Feed = () => {
  ...
  let filteredFeeds = useMemo(() => {
    if (filterParam) {
      return feeds?.filter(feed => feed.post_text.includes(`#${filterParam}`));
    }
    return feeds;
  }, [feeds, filterParam]);
  let perPage = 10;
  ...
  return (
    <div className='flex-1 pt-2'>
      ...
      <div className='feed-posts--wrapper flex flex-col items-center mt-6'>
        {paginatedFeeds.length !== 0 ? paginatedFeeds.map((feed , index) => {
          return (
            <div key={feed.id} className='main-card--wrapper'>
              <PostCard post={feed} index={index} />
            </div>
          )
